...
import { User, Loader2, Mail, ShieldAlert, Eye, TrendingUp, Gift, Users as UsersIcon, Calendar, Phone, Hash, Search, Filter, ChevronLeft, ChevronRight, Shield } from "lucide-react";
...
export function UsersManager() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (roleFilter !== "all") {
                finalUsers = mappedUsers.filter(user => (roleFilter === "admin" && user.isAdmin) ||
    ...
    const roleMutation = useMutation({
        mutationFn: async ({ userId, role }) => {
            return assignRoleFn({ data: { userId, role: role } });
        },
        onSuccess: () => {
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:w-96">
          ...
          <Input placeholder="Search by name, email, or username..." value={searchQuery} onChange={(e) => {
      ...
      <div className="rounded-2xl border border-border/50 bg-card overflow-hidden overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {paginatedUsers.map((user) => (<TableRow key={user.id} className="border-border/40 hover:bg-accent/5 transition-colors">
                ...
                <TableCell className="px-6 py-4">
                  <div className="flex flex-col gap-1">
                    ...
                    {user.phone_number && (<div className="text-[10px] font-bold text-muted-foreground/60">{user.phone_number}</div>)}
                  </div>
                </TableCell>
                <TableCell className="px-6 py-4">
                ...
                <TableCell className="px-6 py-4 text-right">
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg hover:bg-primary/10 hover:text-primary transition-colors" onClick={() => {
      ...
      {totalPages > 1 && (<div className="flex items-center justify-between px-2 py-4 border-t border-border/40">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
            Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, totalCount)} of {totalCount} entries
          </p>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="rounded-xl h-9 px-3 font-bold border-border/50" disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)}>
              <ChevronLeft className="h-4 w-4 mr-1"/>
              Previous
            </Button>
            <div className="flex items-center gap-1">
              {Array.from({ length: totalPages }).map((_, i) => (<Button key={i} variant={currentPage === i + 1 ? "default" : "outline"} size="sm" className={cn("h-9 w-9 rounded-xl font-bold p-0 border-border/50", currentPage === i + 1 && "shadow-md shadow-primary/20")} onClick={() => setCurrentPage(i + 1)}>
                  {i + 1}
                </Button>))}
            </div>
            <Button variant="outline" size="sm" className="rounded-xl h-9 px-3 font-bold border-border/50" disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)}>
              Next
          ...
          {selectedUser && (<div className="relative">
              ...
              <div className="px-6 pb-8">
                <div className="flex flex-col md:flex-row gap-6 -mt-10 mb-8 items-start">
                  ...
                  <div className="pt-10 flex-1 space-y-2">
                    <div className="flex items-center gap-3">
                      ...
                      {selectedUser.isAdmin && (<Badge className="bg-primary text-primary-foreground font-black uppercase text-[10px] px-2 py-0.5 rounded-lg">
                          Admin
                        </Badge>)}
                    </div>
                    <div className="flex flex-wrap gap-4">
                      ...
                      {selectedUser.phone_number && (<div className="flex items-center gap-1.5 text-sm text-muted-foreground font-medium">
                ...
                <div className="mb-8 p-6 rounded-[28px] border border-border/50 bg-accent/5 flex flex-col md:flex-row items-center justify-between gap-6">
                  ...
                  <div className="flex items-center gap-3 w-full md:w-auto">
                    <Select defaultValue={selectedUser.currentRole} onValueChange={(val) => roleMutation.mutate({ userId: selectedUser.id, role: val })} disabled={roleMutation.isPending}>
                ...
                {isDetailsLoading ? (<div className="flex justify-center py-20">
                    ...
                    <Card className="md:col-span-2 border-none shadow-sm bg-card/50 overflow-hidden rounded-2xl">
                      ...
                      <CardContent className="p-0">
                        <div className="max-h-[400px] overflow-y-auto divide-y divide-border/40 scrollbar-thin scrollbar-thumb-primary/10">
                          {userDetails?.transactions.length ? (userDetails.transactions.map((tx) => (<div key={tx.id} className="flex items-center justify-between p-4 hover:bg-accent/5 transition-colors">
                                <div className="space-y-1">
                                ...
                                <div className={cn("font-black text-sm px-2.5 py-1 rounded-lg", tx.amount > 0 ? "bg-green-500/10 text-green-600" : "bg-destructive/10 text-destructive")}>
                                  {tx.amount > 0 ? `+${tx.amount}` : tx.amount}
                                </div>
                              </div>))) : (<div className="p-8 text-center text-muted-foreground text-sm font-medium italic">
                    ...
                    <div className="space-y-6">
                      ...
                      <Card className="border-none shadow-sm bg-card/50 overflow-hidden rounded-2xl">
                        ...
                        <CardContent className="p-0">
                          <div className="max-h-[250px] overflow-y-auto divide-y divide-border/40">
                            {userDetails?.referrals.length ? (userDetails.referrals.map((ref) => (<div key={ref.id} className="p-4 hover:bg-accent/5 transition-colors">
                                ...
                                </div>))) : (<div className="p-8 text-center text-muted-foreground text-xs font-medium">
                                No referrals yet.
                              </div>)}
                          </div>
                        </CardContent>
                      ...
                      <Card className="border-none shadow-sm bg-card/50 overflow-hidden rounded-2xl">
                        ...
                        <CardContent className="p-0">
                          <div className="max-h-[250px] overflow-y-auto divide-y divide-border/40">
                            {userDetails?.redemptions.length ? (userDetails.redemptions.map((red) => (<div key={red.id} className="p-4 hover:bg-accent/5 transition-colors">
                                  <div className="flex items-center justify-between mb-1">
                                    <p className="font-bold text-sm truncate max-w-[120px]">
                                      {red.rewards?.title}
                                    </p>
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className={cn("text-[9px] font-black uppercase tracking-wider", red.status === 'approved' ? "text-green-600" : "text-orange-600")}>
                                      {red.status}
                                    </span>
                                  </div>
                                </div>))) : (<div className="p-8 text-center text-muted-foreground text-xs font-medium">
