...
export function Navigation() {
  ...
  if (isLandingPage) {
    return (
      <>
        <nav className="premium-surface fixed top-3 left-3 right-3 z-50 rounded-2xl bg-card/90 backdrop-blur-xl max-w-[calc(100vw-1.5rem)]">
          <div className="container mx-auto flex h-16 items-center justify-between px-4 sm:h-20">
            ...
            <div className="hidden md:flex items-center gap-8">
              <Link
                to="/about"
                className="text-sm font-black uppercase text-foreground/70 hover:text-foreground"
              >
                About
              </Link>
              <Link
        ...
        <MobileMenuOverlay isOpen={isMobileMenuOpen} onClose={() => setIsMobileMenuOpen(false)}>
          <div className="p-6 flex flex-col h-full">
            ...
            <div className="mt-auto space-y-4">
              <Button
                variant="outline"
                className="w-full font-black uppercase h-12 rounded-xl"
                asChild
              >
              ...
              <Button
                className="w-full font-black uppercase h-12 rounded-xl shadow-lg shadow-primary/20"
                asChild
              >
                <Link to="/auth" onClick={() => setIsMobileMenuOpen(false)}>
                  Get Started
    ...
    <>
      <AlertDialog open={showLogoutDialog} onOpenChange={setShowLogoutDialog}>
        <AlertDialogContent className="rounded-2xl border-border/40 max-w-[400px]">
          ...
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel className="rounded-xl font-bold border-border/40">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <div className="md:hidden fixed top-2 left-2 right-2 z-50 flex items-center justify-between h-20 px-4 bg-card/95 backdrop-blur-md border border-border/40 shadow-xl shadow-black/5 dark:shadow-black/20 rounded-2xl after:absolute after:inset-0 after:rounded-2xl after:shadow-[0_0_15px_rgba(124,58,237,0.5)] after:pointer-events-none after:z-[-1] animate-neon-pulse">
        <div className="flex items-center gap-2">
          <Link
            to="/dashboard"
            className="flex items-center gap-1.5 font-black text-lg uppercase tracking-tighter ml-2"
          >
            <img src="/logo.png" alt="Noble Gain" className="h-8 w-8 object-contain" />
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="relative h-9 w-9 rounded-full p-0 border-2 border-primary/10 shadow-sm ring-2 ring-background"
            ...
            <DropdownMenuContent
              className="w-56 mt-2 rounded-2xl p-2 shadow-xl border-border/40"
              align="end"
            >
              ...
              <DropdownMenuItem
                asChild
                className="rounded-xl focus:bg-primary/5 focus:text-primary cursor-pointer px-3 py-2 font-bold text-sm"
              >
                <Link
                  to="/profile"
              ...
              <DropdownMenuItem
                asChild
                className="rounded-xl focus:bg-primary/5 focus:text-primary cursor-pointer px-3 py-2 font-bold text-sm text-destructive focus:text-destructive focus:bg-destructive/5"
              >
                <button
                  onClick={() => setShowLogoutDialog(true)}
                  className="flex items-center w-full"
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3 bg-primary/5 border border-primary/10 shadow-sm px-4 py-2 rounded-2xl">
            ...
            <span className="text-sm font-black text-foreground">
              ...
              <span className="text-[10px] text-muted-foreground ml-1.5 font-black uppercase tracking-wider">
                Points
              </span>
            </span>
          </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="relative h-9 w-9 rounded-full p-0 border-2 border-primary/10 shadow-sm ring-2 ring-background transition-transform hover:scale-105 active:scale-95"
            ...
            <DropdownMenuContent
              className="w-64 mt-2 rounded-2xl p-2 shadow-xl border-border/40"
              align="end"
            >
              <DropdownMenuLabel className="font-black px-4 py-3">
                <div className="flex flex-col space-y-1">
                  ...
                  <div className="flex items-center gap-1.5 bg-primary/5 w-fit px-2 py-0.5 rounded-lg">
                    ...
                    <p className="text-[10px] text-primary uppercase tracking-wider font-black">
                      {profile?.points_balance?.toLocaleString()} Points
                    </p>
                  </div>
                </div>
              ...
              <DropdownMenuItem
                asChild
                className="rounded-xl focus:bg-primary/5 focus:text-primary cursor-pointer px-3 py-2.5 font-bold text-sm"
              >
              ...
              <DropdownMenuItem
                asChild
                className="rounded-xl focus:bg-destructive/5 focus:text-destructive cursor-pointer px-3 py-2.5 font-bold text-sm text-destructive"
              >
                <button
                  onClick={() => setShowLogoutDialog(true)}
                  className="flex items-center w-full"
