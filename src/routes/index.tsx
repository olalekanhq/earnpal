...
export const Route = createFileRoute("/")({
  head: () => ({
    title: "Noble Gain | Reward Your Time with Real Prizes",
    meta: [
      { name: "description", content: "Join Noble Gain, the ultimate rewards community. Earn points by completing simple tasks, referring friends, and watching ads. Redeem your points for premium gift cards and real prizes today!" },
      { property: "og:title", content: "Noble Gain | Reward Your Time & Earn Real Prizes" },
...
function LandingPage() {
  return <main className="min-h-screen overflow-hidden bg-background">
    <section className="relative isolate px-4 pb-20 pt-32 sm:px-6 md:pb-28 md:pt-40">
      ...
      <div className="container mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1fr_0.9fr] lg:gap-20">
        <motion.div initial="hidden" animate="visible" variants={reveal} className="max-w-2xl">
          ...
          <h1 className="text-balance text-5xl font-black leading-[0.94] tracking-[-0.06em] text-foreground sm:text-6xl md:text-7xl">Your time is worth <span className="text-[#e6c17a]">more.</span></h1>
    ...
    <footer className="border-t border-border bg-card px-4 py-10 sm:px-6"><div className="container mx-auto flex flex-col items-center justify-between gap-7 md:flex-row"><Link to="/" className="flex items-center gap-2 text-xl font-black tracking-tight"><img src="/logo.png" alt="Noble Gain" className="size-8 object-contain" /> <span className="text-foreground">NOBLE <span className="text-[#e6c17a]">GAIN</span></span></Link><div className="flex flex-wrap justify-center gap-x-6 gap-y-3 text-xs font-bold uppercase tracking-widest text-muted-foreground"><Link to="/" className="hover:text-primary">Home</Link><Link to="/about" className="hover:text-primary">About</Link><Link to="/earn" search={{ tab: "tasks" }} className="hover:text-primary">Earn</Link><Link to="/redeem" className="hover:text-primary">Redeem</Link><Link to="/privacy" className="hover:text-primary">Privacy</Link><Link to="/terms" className="hover:text-primary">Terms</Link><Link to="/auth" className="hover:text-primary">Login</Link></div><p className="text-center text-xs font-medium text-muted-foreground">© 2026 Noble Gain. All rights reserved.</p></div></footer>
  </main>;
}
