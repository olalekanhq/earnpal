...
function RedeemPage() {
  ...
  const filteredRewards =
    activeCategory === "All" ? rewards : rewards?.filter((r: any) => r.category === activeCategory);
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      toast.success(
        "Redemption request submitted! Your reward will be sent to your registered email address after confirmation.",
      );
  ...
  return (
    <>
      <div className="premium-enter flex w-full flex-col gap-8">
        <header className="flex flex-col justify-between gap-6 border-b border-border/70 pb-6 md:flex-row md:items-end">
          <div className="space-y-1">
            <h1 className="text-3xl font-black tracking-tight text-foreground">Redeem Rewards</h1>
            <p className="text-muted-foreground font-medium">
              Exchange your points for amazing gift cards and rewards.
            </p>
          </div>
            ...
            {categories.map((cat) => (
              ...
              >
                <cat.icon
                  className={cn(
                    "mr-2 h-4 w-4",
                    activeCategory === cat.name ? "text-primary-foreground" : "text-primary",
                  )}
                />
                {cat.name}
              </Button>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredRewards?.length
            ? filteredRewards.map((reward) => (
                <Card
                  key={reward.id}
                  className="premium-surface group flex flex-col overflow-hidden bg-card transition-all hover:-translate-y-0.5 hover:shadow-xl"
                >
                  ...
                  <CardHeader className="pb-4">
                    <div className="flex justify-between items-start mb-2">
                      <Badge
                        variant="secondary"
                        className="bg-primary/5 text-primary border-none rounded-lg px-2 py-0.5 font-bold uppercase text-[9px]"
                      >
                    ...
                    <CardDescription className="text-sm font-medium line-clamp-2 mt-1">
                      {reward.description}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="mt-auto pt-0 pb-6 px-6">
                    <Button
                      ...
                      variant={
                        (profile?.points_balance || 0) < reward.cost_points ? "outline" : "default"
                      }
        ...
        <DialogContent className="sm:max-w-[425px] rounded-[2rem]">
          <DialogHeader>
            ...
            <DialogDescription className="font-medium">
              ...
              <span className="text-foreground font-bold">{selectedReward?.title}</span> for{" "}
          ...
          <div className="py-6 flex flex-col items-center justify-center space-y-4">
            ...
            <div className="text-center">
              ...
              <p className="text-3xl font-black">
                {(
                  (profile?.points_balance || 0) - (selectedReward?.cost_points || 0)
                ).toLocaleString()}{" "}
