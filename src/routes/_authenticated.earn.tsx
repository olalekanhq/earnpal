    ...
    meta: [
      {
        name: "description",
        content:
          "Discover multiple ways to earn points on Noble Gain. Complete social media tasks, watch video ads, and participate in surveys to boost your balance today.",
      },
      { property: "og:title", content: "Earn Points | Tasks & Rewards | Noble Gain" },
      {
        property: "og:description",
        content:
          "Unlock daily earning opportunities. Complete simple tasks and watch your points grow in real-time.",
      },
...
function EarnPage() {
  ...
  const filteredTasks = (tasks as any[])?.filter((t: any) => {
    ...
    const matchesCategory =
      activeStatus !== "available" || activeCategory === "All" || t.category === activeCategory;
    ...
    <div className="premium-enter flex w-full flex-col gap-8">
        ...
        <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Add your required social handles in your profile to start earning points.
            </p>
          </div>
          <Button
            variant="outline"
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
          <div className="flex max-w-full overflow-x-auto rounded-2xl border border-border/70 bg-card p-1 shadow-sm">
            <button
        ...
        <div className="grid grid-cols-1 gap-6 sm:gap-6 md:grid-cols-2 lg:grid-cols-3 w-full max-w-7xl mx-auto px-1">
          {filteredTasks?.length
            ? (filteredTasks as any[]).map((task: any) => (
                <Card
                  key={task.id}
                  className="premium-surface group flex w-full flex-col overflow-hidden bg-card transition-all hover:-translate-y-0.5 hover:shadow-xl"
                >
                  ...
                  <CardHeader className="p-5 sm:p-4 pb-2 sm:pb-4">
                    <div className="flex justify-between items-start mb-2 sm:mb-3">
                      <Badge
                        variant="secondary"
                        className="bg-primary/5 text-primary border-none rounded-lg px-1.5 sm:px-2.5 py-0.5 font-bold uppercase text-[10px]"
                      >
                        {task.category}
                      </Badge>
                    ...
                    <CardDescription className="text-[11px] sm:text-sm font-medium line-clamp-2 mt-0.5 sm:mt-1">
                      {task.description}
                    </CardDescription>
                      ...
                      task.status !== "verified" && (
                        <div className="mt-2 sm:mt-3 space-y-1 sm:space-y-2">
                          <div className="flex justify-between text-[8px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-widest text-muted-foreground">
                            <span>Progress</span>
                            <span>
                              {task.watch_count || 0}/{task.video_ad_count}
                            </span>
                          </div>
                          ...
                          <p className="text-[8px] sm:text-[10px] text-muted-foreground italic font-medium hidden xs:block">
                            Earn {task.points} pts after {task.video_ad_count} watches.
                          </p>
                        </div>
                      )}
                  </CardHeader>
                  <CardContent className="mt-auto pt-0 pb-3 sm:pb-6 px-3 sm:px-6">
                    ...
                    <Button
                      ...
                      disabled={
                        ...
                        taskUiStates[task.id] === "submitting"
                      }
                      onClick={async () => {
                        const {
                          data: { user },
